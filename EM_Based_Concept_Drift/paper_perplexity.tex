...
In the streaming setting, EM-MTE processes data in an online fashion. When a new block arrives, it can be assigned to an existing task or treated as a new task. The responsibilities $\gamma_{ik}^{(r)}$ are updated incrementally, allowing the model to:

\begin{itemize}
	\item Quickly increase the weight of components that perform well on the new data distribution (abrupt drift).
	\item Gradually shift responsibilities when the change is slow (gradual drift).
	\item Reactivate previously useful components when patterns reappear (recurring drift).
\end{itemize}

This mechanism serves as an implicit drift detector: a significant change in the distribution of responsibilities $\pi_k^{(r)}$ across blocks signals concept drift without requiring a separate detection module.
...
The per-sample time complexity of EM-MTE is $\mathcal{O}(K \cdot c)$, where $c$ is the cost of a single component update (usually constant or linear in dimension $d$). Memory usage is $\mathcal{O}(K \cdot m + T \cdot K)$, where $m$ is the memory required by one component. This is significantly more efficient than maintaining separate models per task and competitive with popular ensembles such as DWM while providing stronger theoretical guarantees.
