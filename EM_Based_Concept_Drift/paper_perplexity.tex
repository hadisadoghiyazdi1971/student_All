...
\title{\textbf{An Online Ensemble Method for Detecting Concept Drift in a Data Stream}}

\author{
	Emad Mahmodi$^{1}$, \quad Hadi Sadoghi Yazdi$^{2}$, \quad Abbas Ghaemi Bafghi$^{3}$ \\[6pt]
	\small $^{1,2,3}$Department of Computer Engineering, Ferdowsi University of Mashhad, Iran \\[4pt]
	\small \texttt{emad.mahmodi@mail.um.ac.ir}
}

\date{Received: ---\quad Revised: ---\quad Accepted: ---}
...
\begin{document}
	...
	\begin{abstract}
		Today's incoming data has been expanding rapidly, and a streaming classifier is increasingly used to cope with essential processing requirements, i.e., rapid prediction and high performance with limited resources. Streaming mining techniques must be adapted to the sudden change of data. To deal with these problems, in this paper, we propose an \textbf{EM-Based Multi-Task Ensemble (EM-MTE)} for concept-drifting streaming data. Online learning algorithms are treated as shared components, while drift-sensitive objectives are treated as tasks. The proposed method combines task outputs through latent responsibility variables and estimates task-specific component weights by the Expectation-Maximization (EM) algorithm. Performance of our approach is evaluated against related streaming algorithms using benchmark and real datasets from the UCI repository and malicious web pages respectively. The main advantage of our approach is its adaptive task combination with fast functionality, low resource usage, and usability in real-life applications for streaming data.
