...
def choose_query(
    ...
    if method == "TC-noReliability":
        score = uncertainty * (1.0 + alpha_density * density[unlabeled])
        return int(unlabeled[int(np.argmax(score))])

    if method == "TC-noDensity":
        reliability = 1.0 / (1.0 + eta_radius * radii[unlabeled])
        score = uncertainty * reliability
        return int(unlabeled[int(np.argmax(score))])

    if method == "PMARLDS-lite":
        finite_bag_radius = 1.0 / np.sqrt(np.maximum(1.0, bag_sizes[unlabeled]))
        reliability = 1.0 / (1.0 + eta_radius * finite_bag_radius)
...
def run_active_learning(
    ...
    bag_sizes = data.bag_sizes_train
    ...
    runtimes: Dict[str, float] = {m: 0.0 for m in methods}
    ...
    for _ in range(len(np.unique(data.y_train)), budget + 1):
        for method in methods:
            ...
            if len(L) < budget:
                ...
                q = choose_query(method, clf, Z_train, unlabeled, radii_train, bag_sizes, density, rng)
                L.append(q)
            runtimes[method] += time.perf_counter() - t0
    return curves, runtimes
...
def repeated_experiment(
    ...
    for r in range(repeats):
        ...
        curves, elapsed_by_method = run_active_learning(data, seed + 10_000 + r, budget, gamma=gamma, n_features=n_features)
        for method, vals in curves.items():
            all_curves.setdefault(method, []).append(vals)
            runtimes.setdefault(method, []).append(elapsed_by_method[method])
            repeat_rows.append({
                "dataset": name,
                "repeat": r,
                "method": method,
                "final_accuracy": float(vals[-1]),
                "AULC": float(np.mean(vals)),
                "runtime_seconds": float(elapsed_by_method[method]),
            })
    ...
    with open(outdir / f"{name}_repeat_metrics.csv", "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["dataset", "repeat", "method", "final_accuracy", "AULC", "runtime_seconds"])
        w.writeheader()
