...
def make_gaussdist(seed: int, dim: int = 10, n_per_class: int = 120, test_ratio: float = 0.3,
                   variable_bag: bool = True, mean_gap: float = 1.0,
                   covariance_multiplier: float = 1.0) -> BagDataset:
    ...
    def one_bag(label: int) -> np.ndarray:
        ...
        center = np.ones(dim) if label == 1 else (1.0 + mean_gap) * np.ones(dim)
        mean = rng.normal(loc=center, scale=np.sqrt(0.5), size=dim)
        cov = _spd(rng, dim, covariance_multiplier * (0.6 if label == 1 else 1.2))
