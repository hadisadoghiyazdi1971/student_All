...
\end{equation}
and the estimated-set rule of Eq.~\eqref{eq:estimated_set} is unchanged, $\widehat{\mathcal{S}}(x) = \mathcal{S}_{\widehat{j}}$ with $\widehat{j} = \arg\max_j R_j(x)$. Because $\theta_j$ is now unknown, the posterior $P(c_j \mid x)$ cannot be evaluated in closed form as in Eq.~\eqref{eq:posterior_component}; instead, it is \emph{jointly estimated} together with $\theta_j$ through a relaxed, fuzzy surrogate $u_{ij} \in [0,1]$ that plays the same role as $P(c_j\mid x_i)$ but is optimized rather than computed. This yields the maximum a posteriori training objective
\[
\max_{u,\,\theta} \; \sum_{j=1}^{K} \sum_{i=1}^{N} u_{ij}^{m} \, R_j(x_i), \qquad 1 < m < \infty,
\]
which is exactly the quantity developed below, first for fixed items (Section~\ref{subsec:item_clustering}) and then for streaming users, where the candidate-set descriptors $\theta_j$ are updated online as new evidence arrives (Section~\ref{subsec:user_clustering}). Sections~\ref{subsec:distributed_clustering}--\ref{subsec:theory} extend this instantiation with graph-based smoothing of the estimated $\theta_j$, a rating-prediction rule built directly on $R_j(x)$, and convergence guarantees for the resulting alternating estimator.

\subsection{Prototype Discovery via Fuzzy Item Clustering}
\label{subsec:item_clustering}

As a first, static instantiation of Eq.~\eqref{eq:collapsed_relevance}, items are assumed fixed and are clustered using the Fuzzy C-Means (FCM) algorithm, which recovers a set of item-side prototypes analogous to $\{\mathbf{m}_i\}$ in the general framework. The objective function is
\begin{equation}
	J = \sum_{k=1}^{C_i} \sum_{i=1}^{n} u_{ki}^m \| x_i - \mu_k \|^2,
\end{equation}
where $u_{ki} \in [0,1]$ is the degree of membership of item $x_i$ in cluster $k$, $\mu_k$ is the corresponding centroid, $n$ is the number of items, $C_i$ is the number of item clusters, and $m > 1$ is the fuzzifier. Differentiating $J$ with respect to $u$ and $\mu$ yields the standard closed-form membership and centroid updates. These item-side clusters serve as fixed reference prototypes for the streaming user-side instantiation developed next.
...
\end{equation}
where $f(z_i \mid \theta_j)$ is the likelihood of user $i$'s latent vector under set $j$'s descriptor, and $f(\theta_j)$ is the prior over that descriptor — the same two Bayesian ingredients as $p(\mathbf{z}(x)\mid c_i)$ and $\rho_i$ in Section~\ref{subsec:latent_filtering}, now instantiated for a descriptor that must itself be estimated. Assuming both are Gaussian,
\begin{align}
...
\begin{figure}[t]
	...
	\begin{tikzpicture}[
		...
		% --- Offline / static branch (Section 3.3) ---
		\node[box] (items) {Fixed items\\ $\{x_i\}$};
		\node[box, below=of items] (fcm) {FCM prototype\\ discovery (\S\ref{subsec:item_clustering})};
		
		% --- Streaming branch (Section 3.4) ---
		...
		% --- prior feedback loop (streaming / online nature)
...
The objective $J_m$ (Eq.~\eqref{eq:obj_function}) is optimized by alternating between updating the membership matrix $u$ (Eq.~\eqref{eq:update_u}) and updating the set descriptors $\theta_j$ (Eqs.~\eqref{eq:update_mean}--\eqref{eq:update_cov}), holding the other block fixed at each step — a block-coordinate (alternating) minimization scheme in the same family as classical FCM. Convergence follows from the generalized convergence theorem of Zangwill together with the local-convergence result of \cite{hathaway1986local}: provided (i) each subproblem — minimizing $J_m$ over $u$ with $\theta$ fixed, and over $\theta$ with $u$ fixed — has a unique, continuous solution (guaranteed here, since Eq.~\eqref{eq:update_u} and Eqs.~\eqref{eq:update_mean}--\eqref{eq:update_cov} are closed-form and continuous in their inputs), and (ii) the iterates are initialized sufficiently close to a stationary point of $J_m$, the sequence $\{u^{(k)}, \theta^{(k)}\}$ converges to a local minimum of $J_m$.
