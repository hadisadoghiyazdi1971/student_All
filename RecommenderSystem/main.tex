...
This study proposes a Bayesian clustering-based recommendation method that efficiently handles streaming data and reduces data sparsity. First, items are clustered using FCM to minimize dimensionality for user clustering. As users enter the system continuously, a Bayesian clustering approach is applied, where cluster centroids influence each other based on their proximity, mitigating the impact of outliers. Learning is distributed, ensuring adaptability without the need to store all data. User ratings from neighboring clusters are then leveraged to predict unseen ratings and recommend top-scoring items. The Bayesian clustering method enhances scalability by updating cluster parameters incrementally, eliminating the need for full re-clustering when new data arrives. This makes the system suitable for dynamic environments. Additionally, to address the cold-start problem, the model incorporates user attributes such as age, gender, and occupation, enriching clustering beyond rating data. Users are represented as vectors containing both scores and personal characteristics, improving recommendation accuracy in early interactions.
