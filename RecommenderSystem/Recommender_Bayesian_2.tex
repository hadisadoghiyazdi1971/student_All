...
The proposed method is built on a Bayesian latent filtering view of recommendation. Instead of directly assigning an incoming entity \(x\) to a recommendation set, the model first maps \(x\) into a latent representation, filters this representation through hidden components, and then decodes the activated hidden components into set-level recommendation evidence. The resulting inference chain is
\[
x
\longrightarrow
\mathbf z(x)=\phi(x)
...
\mathcal S_j .
\]
Here, \(x\) may denote an incoming user, item, or user--item representation, depending on the recommendation scenario. The variable \(c_i\) denotes the \(i\)-th hidden component, while \(\mathcal S_j\) denotes the \(j\)-th candidate recommendation set. The descriptor \(\theta_j\) characterizes the latent distribution of \(\mathcal S_j\).

This formulation is not a conventional cluster-then-recommend strategy. In a hard clustering recommender, \(x\) is assigned to one cluster and the recommendation is obtained from that cluster only. In contrast, the proposed method marginalizes over all hidden components. Therefore, uncertainty about the latent state of \(x\) is preserved until the final recommendation stage.
...
The incoming entity \(x\in\mathcal X\) is first encoded into a latent vector
\[
\mathbf z(x)=\phi(x)\in\mathbb R^d,
\]
where \(\phi:\mathcal X\rightarrow\mathbb R^d\) is an encoder. The encoder can be a fixed feature map, a dimensionality-reduction map, a matrix-factorization embedding, or a neural encoder. The proposed probabilistic formulation does not require a specific choice of \(\phi\); it only requires that \(x\) be represented in a latent space where Bayesian filtering can be performed.

We introduce \(C\) hidden components
\[
c_i,\qquad i=1,\ldots,C.
\]
Each hidden component \(c_i\) is represented by a prototype and covariance
\[
\psi_i=(\mathbf m_i,\Lambda_i),
\]
where \(\mathbf m_i\in\mathbb R^d\) is the prototype of the hidden component and \(\Lambda_i\) is its covariance. Thus, \(c_i\) is the hidden event, while \(\mathbf m_i\) is its continuous representative in the latent space.

The likelihood of the encoded input under hidden component \(c_i\) is modeled as
\[
p(\mathbf z(x)\mid c_i)
=
\mathcal N\big(\mathbf z(x);\mathbf m_i,\Lambda_i\big).
\]
Therefore,
\begin{equation}
...
\begin{equation}
	...
	=
	\frac{
		\rho_i\,\mathcal N(\mathbf z(x);\mathbf m_i,\Lambda_i)
	}{
		\sum_{\ell=1}^{C}
		\rho_\ell\,\mathcal N(\mathbf z(x);\mathbf m_\ell,\Lambda_\ell)
	},
\end{equation}
where
\[
...
\sum_{i=1}^{C}\rho_i=1.
\]
The quantity \(q_i(x)\) is the Bayesian latent filter. It measures how strongly the incoming entity \(x\) activates the hidden component \(c_i\).
...
After latent filtering, the model evaluates how strongly each hidden component supports a candidate recommendation set. Since \(c_i\) is represented by its prototype \(\mathbf m_i\), the likelihood of the prototype under the descriptor of set \(\mathcal S_j\) is defined as
\begin{equation}
...
\begin{equation}
	...
	\right).
\end{equation}
The negative sign in the exponential is essential; otherwise the expression is not a valid Gaussian density.

Using Bayes' rule, the posterior association between the set descriptor \(\theta_j\) and the hidden component \(c_i\) is
\begin{equation}
...
\end{equation}
Here, \(P(\theta_j)\) is the prior probability of the candidate set descriptor. It may be uniform when no prior preference over sets is available, or it may be estimated from historical popularity, set size, or temporal frequency in streaming data.

The relevance score of \(x\) to the candidate set \(\mathcal S_j\) is obtained by marginalizing over all hidden components:
\begin{equation}
...
Equation~\eqref{eq:relevance_score} is the central probabilistic mechanism of the proposed method. The term \(P(c_i\mid x)\) explains how the input activates the internal latent components, while \(f(\theta_j\mid c_i)\) explains how each activated component supports a candidate recommendation set.

% ------------------------------------------------------------
\subsection{Encoder--Latent Filter--Set Decoder Structure}
\label{subsec:tikz_structure}
% ------------------------------------------------------------

\begin{figure}[t]
	\centering
	\begin{tikzpicture}[
		node distance=1.45cm and 1.55cm,
		box/.style={
			draw,
			rounded corners,
			align=center,
		...
		\node[box] (x) {Incoming entity\\ \(x\)};
		\node[box, right=of x] (enc) {Latent encoder\\ \(\mathbf z(x)=\phi(x)\)};
		\node[box, right=of enc] (filter) {Bayesian filter\\ \(P(c_i\mid x)\)};
		
		\node[smallbox, above right=0.60cm and 0.95cm of filter] (c1) {Component\\ \(c_1,\mathbf m_1\)};
		\node[smallbox, right=0.95cm of filter] (ci) {Component\\ \(c_i,\mathbf m_i\)};
		\node[smallbox, below right=0.60cm and 0.95cm of filter] (cC) {Component\\ \(c_C,\mathbf m_C\)};
		
		\node[box, right=1.75cm of ci] (decoder) {Set decoder\\ \(f(\theta_j\mid c_i)\)};
		\node[box, right=of decoder] (score) {Relevance\\ \(R_j(x)\)};
		...
		\node[font=\small, above=0.15cm of latentbox]
		{Encoder--latent filter--set decoder};
		
		\node[font=\small, below=0.30cm of filter]
		{\(q_i(x)=P(c_i\mid x)\)};
		
		\node[font=\small, below=0.30cm of decoder]
		{\(f(\theta_j\mid c_i)\)};
		
		\node[font=\small, below=0.30cm of score]
		{\(R_j(x)=\sum_i P(c_i\mid x)f(\theta_j\mid c_i)\)};
		
	\end{tikzpicture}
	\caption{Bayesian latent filtering view of the proposed recommender. The structure is encoder-like because the input \(x\) is first mapped into a latent representation and then filtered through hidden components. It is not a classical autoencoder, because the objective is not to reconstruct \(x\), but to infer the most relevant recommendation set \(\widehat{\mathcal S}(x)\).}
...
The latent component \(c_i\) is not introduced merely as a computational device. It provides a probabilistic factorization of the recommendation decision. Let \(Y\) be the set-relevance decision variable:
\[
Y=j
...
\end{equation}
We assume that the latent component \(c_i\) provides a sufficient intermediate representation of \(x\) for set-level inference:
\begin{equation}
...
\begin{proposition}[Bayesian optimality of the marginalized decision rule]
	...
	\arg\max_j R_j(x)
	\]
	is the maximum a posteriori set estimator.
\end{proposition}
...
\begin{proof}
	...
	\arg\max_j R_j(x),
	\]
	which proves that the proposed rule is the MAP estimator of the relevant set.
\end{proof}
...
The latent components and set descriptors must be estimated from data. Let
\[
\mathcal B_t=\{x_1^{(t)},x_2^{(t)},\ldots,x_{n_t}^{(t)}\}
\]
...
The effective mass of component \(c_i\) in the new mini-batch is
\begin{equation}
	...
	q_{ni}^{(t)}.
\end{equation}
With a forgetting factor \(\lambda\in(0,1]\), the streaming component mass is updated as
\begin{equation}
	...
	M_i^{(t)}.
\end{equation}
The prior probability of component \(c_i\) is then updated by
\begin{equation}
	\label{eq:rho_update}
	\rho_i^{(t+1)}
	=
	\frac{N_i^{(t+1)}}{\sum_{\ell=1}^{C}N_\ell^{(t+1)}}.
\end{equation}

The component prototype is updated as
\begin{equation}
...
\begin{equation}
	...
	\frac{
		...
		\sum_{n=1}^{n_t}
		q_{ni}^{(t)}
		\left(\mathbf z_n^{(t)}-\mathbf m_i^{(t+1)}\right)
		\left(\mathbf z_n^{(t)}-\mathbf m_i^{(t+1)}\right)^T
	}{
		N_i^{(t+1)}+\varepsilon
	}
	+
	\delta I,
\end{equation}
where \(\delta I\) is a small diagonal regularizer that guarantees positive definiteness.

The descriptor of each candidate set \(\mathcal S_j\) is computed from the latent evidence of its members. Let \(x\in\mathcal S_j\) denote an entity belonging to set \(\mathcal S_j\). The set mean is estimated as
\begin{equation}
...
\end{equation}
The set covariance is estimated as
\begin{equation}
	...
	\delta I.
\end{equation}
Equations~\eqref{eq:set_mean_update} and \eqref{eq:set_covariance_update} show how the set descriptor \(\theta_j=(\mu_j,\Sigma_j)\) is obtained. Therefore, \(\mathcal S_j\) is not treated as a vague label; it is represented by a probability distribution in the latent space.
...
For recommendation, the set-level relevance score must be used directly in the prediction stage. Otherwise, the method would collapse into a conventional cluster-average recommender. We therefore define a normalized set relevance weight:
\begin{equation}
	...
	\frac{
		...
		\sum_{\ell=1}^{K}R_\ell(x)+\varepsilon
	}.
\end{equation}

Because each set descriptor has its own uncertainty, we define a confidence factor
\begin{equation}
	\label{eq:set_confidence}
	\omega_j
	=
	\frac{1}{1+\operatorname{tr}(\Sigma_j)}.
\end{equation}
A compact set descriptor produces a larger confidence value, while an uncertain descriptor is automatically down-weighted.

The final uncertainty-aware gating coefficient is
\begin{equation}
	...
	\frac{
		...
		\sum_{\ell=1}^{K}\alpha_\ell(x)\omega_\ell+\varepsilon
	}.
\end{equation}

Let \(u\) be the target user and \(a\) a candidate item. In the user-based collaborative filtering instantiation, \(x=x_u\) is the representation of user \(u\), and \(\mathcal S_j\) is a candidate user set. For each set \(\mathcal S_j\), define
\[
\mathcal N_j(u,a)
=
...
\end{equation}
The within-set latent similarity is defined as
\begin{equation}
	...
	\exp\left(
	-\frac{1}{2}
	(\mathbf z_u-\mathbf z_v)^T
	\Sigma_j^{-1}
	(\mathbf z_u-\mathbf z_v)
	\right),
\end{equation}
where
\[
...
This prediction rule is fundamentally different from a simple cluster-average predictor. A cluster-average predictor first selects a cluster and then averages ratings inside that cluster. In contrast, Eq.~\eqref{eq:bayesian_rating_prediction} uses the full posterior chain
\[
x_u
\longrightarrow
P(c_i\mid x_u)
...
\widehat r_{ua}.
\]
Thus, the final rating is an uncertainty-aware Bayesian mixture over recommendation sets.

For top-\(N\) recommendation, the predicted rating is computed for every unrated item \(a\in\mathcal I\setminus\mathcal I_u\), and the \(N\) items with the largest predicted scores are returned:
\begin{equation}
	\label{eq:topn_rule}
	\operatorname{TopN}(u)
	=
...
where
\[
a_k\in\mathcal I\setminus\mathcal I_u.
\]
...
The complete streaming procedure is summarized in Algorithm~\ref{alg:bayesian_latent_filtering}. The algorithm receives data sequentially, updates the latent components through Bayesian responsibilities, updates the set descriptors, and then performs set-level or rating-level recommendation.

\begin{algorithm}[H]
	\caption{Bayesian Latent Filtering for Streaming Recommendation}
	\label{alg:bayesian_latent_filtering}
	\begin{algorithmic}[1]
		\REQUIRE Streaming mini-batches \(\{\mathcal B_t\}_{t=1}^{T}\), number of hidden components \(C\), candidate sets \(\{\mathcal S_j\}_{j=1}^{K}\), encoder \(\phi\), forgetting factor \(\lambda\), regularizers \(\varepsilon,\delta\).
		\ENSURE Updated latent components \(\{c_i\}_{i=1}^{C}\), set descriptors \(\{\theta_j\}_{j=1}^{K}\), relevance scores \(R_j(x)\), and recommendations.
		
		\STATE Initialize \(\rho_i^{(0)}\), \(\mathbf m_i^{(0)}\), \(\Lambda_i^{(0)}\), and \(N_i^{(0)}\) for all \(i=1,\ldots,C\).
		\STATE Initialize \(\theta_j^{(0)}=(\mu_j^{(0)},\Sigma_j^{(0)})\) for all \(j=1,\ldots,K\).
		...
		\mathbf z_n^{(t)}=\phi(x_n^{(t)}).
		\]
		\STATE Compute posterior component responsibilities:
		\[
		q_{ni}^{(t)}=P(c_i\mid x_n^{(t)})
		\]
		using Eq.~\eqref{eq:responsibility_stream}.
		\ENDFOR
		...
		\STATE Update set descriptor \(\theta_j=(\mu_j,\Sigma_j)\) using Eqs.~\eqref{eq:set_mean_update}--\eqref{eq:set_covariance_update}.
		\ENDFOR
		
		\FOR{each query entity \(x\)}
		\STATE Compute \(P(c_i\mid x)\) for all hidden components.
		\STATE Compute \(f(\theta_j\mid c_i)\) for all candidate sets.
		\STATE Compute \(R_j(x)\) using Eq.~\eqref{eq:relevance_score}.
		\STATE Estimate the most relevant set:
		\[
		\widehat{\mathcal S}(x)=\mathcal S_{\arg\max_j R_j(x)}.
		\]
		\ENDFOR
		
		\STATE For rating prediction, compute \(\widehat r_{ua}\) using Eq.~\eqref{eq:bayesian_rating_prediction}.
		\ENDFOR
	\end{algorithmic}
\end{algorithm}
...
The latent filtering stage can be interpreted as an online maximum-likelihood estimation problem for a Gaussian mixture in the latent space. For a mini-batch \(\mathcal B_t\), define the negative log-likelihood
\begin{equation}
	...
	\right].
\end{equation}
The posterior responsibility in Eq.~\eqref{eq:responsibility_stream} is exactly the E-step responsibility of this latent-variable model. The updates in Eqs.~\eqref{eq:streaming_mass}--\eqref{eq:covariance_update} are streaming M-step updates with forgetting.
...
\begin{theorem}[Monotone improvement of the latent filtering likelihood]
	...
	\delta>0,
	\]
	and the responsibilities \(q_{ni}^{(t)}\) are computed by Eq.~\eqref{eq:responsibility_stream}. Then each exact EM update of \(\rho_i\), \(\mathbf m_i\), and \(\Lambda_i\) for a fixed mini-batch \(\mathcal B_t\) does not increase the negative log-likelihood \(\mathcal L_t\). Equivalently,
	\[
	\mathcal L_t^{(k+1)}
	\leq
	\mathcal L_t^{(k)}.
...
\begin{proof}
	For fixed parameters, the responsibilities \(q_{ni}^{(t)}\) form a valid posterior distribution over hidden components:
	\[
	q_{ni}^{(t)}\geq 0,
	\qquad
	\sum_{i=1}^{C}q_{ni}^{(t)}=1.
	\]
	Using Jensen's inequality, the negative log-likelihood in Eq.~\eqref{eq:nll_objective} is upper-bounded by the standard EM free-energy functional. The E-step sets \(q_{ni}^{(t)}\) equal to the posterior responsibilities, which makes the bound tight. The M-step minimizes this bound with respect to \(\rho_i\), \(\mathbf m_i\), and \(\Lambda_i\). Therefore, the likelihood cannot decrease, or equivalently the negative log-likelihood cannot increase. The covariance regularization \(\Lambda_i\succeq\delta I\) prevents singular covariance matrices and keeps the objective well-defined.
\end{proof}

\begin{corollary}[Convergence of the inner latent update]
	\label{cor:inner_convergence}
	If the latent representations \(\mathbf z_n\) are bounded and covariance matrices are regularized by \(\delta I\), then the sequence of negative log-likelihood values generated by the inner latent filtering update is monotonically non-increasing and bounded below. Therefore, it converges to a finite value.
\end{corollary}

The theorem establishes convergence of the probabilistic latent filtering stage. This is stronger and cleaner than relying on a positive-definite Hessian argument for a fuzzy clustering objective. The proposed method is therefore supported by a standard latent-variable likelihood principle rather than by a purely heuristic clustering update.

% ------------------------------------------------------------
\subsection{Discussion: Why the Method Is Not Ordinary Clustering}
\label{subsec:not_ordinary_clustering}
% ------------------------------------------------------------

The proposed model differs from ordinary clustering-based recommendation in three essential ways.

First, the assignment of \(x\) to hidden components is not hard. The model computes a posterior distribution
\[
P(c_i\mid x),
\]
which preserves uncertainty over all hidden components.

Second, the final recommendation set is not selected directly from a single cluster. Instead, set relevance is computed by marginalization:
\[
R_j(x)
=
\sum_{i=1}^{C}
P(c_i\mid x)f(\theta_j\mid c_i).
\]
Thus, the hidden layer acts as a Bayesian mediator between incomplete observations and recommendation-set inference.
